...
import ToastPersist from '@/components/toasts/ToastPersist.vue';
import ToastWin from '@/components/toasts/ToastWin.vue';
import ToastWrong from '@/components/toasts/ToastWrong.vue';
import ToastNewGame from '@/components/toasts/ToastNewGame.vue';
...
async function newGame(rowCount, colCount) {
	doneLoading.value = false;
	mode.value = 'road';
	await db.currentGame.clear();
...
async function newGameBtn(e, rowCount, colCount) {
	if (newGameToastVisible) return;
	const placedCells = game.getFlatCells().filter(c => c.displayValue !== null && c.displayValue !== 'charge');
	if (!game.startTime || (game.startTime && game.endTime) || placedCells.length === 0) {
		newGame(rowCount, colCount);
		return;
	}
	newGameToastVisible = true;
	const toastOptions = {
		component: ToastNewGame,
		listeners: {
			yes: () => {
				console.log('User confirmed new game');
				newGameToastVisible = false;
				newGame(rowCount, colCount);
			},
			no: () => {
				console.log('User denied new game');
				newGameToastVisible = false;
			},
		},
	};
